  ...
  Linux)
    if command -v apk >/dev/null 2>&1; then
      apk add --no-cache \
        ...
        gettext-static git glib-dev glib-static libffi-dev \
      ...
      sudo apt-get install -y \
        ...
        gettext libffi-dev libglib2.0-dev libpcre2-dev meson ninja-build \
        pkg-config python3 python3-venv zlib1g-dev
    else
